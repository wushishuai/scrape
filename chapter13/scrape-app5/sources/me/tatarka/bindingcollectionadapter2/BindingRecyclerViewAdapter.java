...
public class BindingRecyclerViewAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements BindingCollectionAdapter<T> {
    ...
    public final void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
        onBindBinding(DataBindingUtil.getBinding(viewHolder.itemView), this.itemBinding.variableId(), this.itemBinding.layoutRes(), position, this.items.get(position));
    }

    @Override // android.support.v7.widget.RecyclerView.Adapter
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position, List<Object> payloads) {
        if (isForDataBinding(payloads)) {
            DataBindingUtil.getBinding(holder.itemView).executePendingBindings();
        } else {
    ...
    private boolean isForDataBinding(List<Object> payloads) {
        ...
        for (int i = 0; i < payloads.size(); i++) {
            if (payloads.get(i) != DATA_INVALIDATION) {
                return false;
            }
        }
        return true;
    ...
    public long getItemId(int position) {
        ...
        return itemIds == null ? (long) position : itemIds.getItemId(position, (T) this.items.get(position));
    }
