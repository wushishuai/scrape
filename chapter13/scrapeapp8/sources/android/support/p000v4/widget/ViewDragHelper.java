...
public class ViewDragHelper {
    ...
    public static abstract class Callback {
        ...
        public void onViewPositionChanged(@NonNull View changedView, int left, int top, @AbstractC0065Px int dx, @AbstractC0065Px int dy) {
        }
    ...
    private int computeSettleDuration(View child, int dx, int dy, int xvel, int yvel) {
        ...
        float yweight = f4 / f3;
        return (int) ((((float) computeAxisDuration(dx, xvel2, this.mCallback.getViewHorizontalDragRange(child))) * xweight) + (((float) computeAxisDuration(dy, yvel2, this.mCallback.getViewVerticalDragRange(child))) * yweight));
    }
    ...
    protected boolean canScroll(@NonNull View v, boolean checkV, int dx, int dy, int x, int y) {
        ...
        if (!checkV || (!v.canScrollHorizontally(-dx) && !v.canScrollVertically(-dy))) {
            return false;
        }
        return true;
    }
