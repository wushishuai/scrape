...
public class NotificationCompat {
    ...
    public static class Builder {
        ...
        private Bitmap reduceLargeIconSize(Bitmap icon) {
            ...
            int maxWidth = res.getDimensionPixelSize(C0070R.dimen.compat_notification_large_icon_max_width);
            int maxHeight = res.getDimensionPixelSize(C0070R.dimen.compat_notification_large_icon_max_height);
    ...
    public static abstract class Style {
        ...
        */
        ...
        private Bitmap createIconWithBackground(int iconId, int size, int iconSize, int color) {
            Bitmap coloredBitmap = createColoredBitmap(C0070R.C0072drawable.notification_icon_background, color == 0 ? 0 : color, size);
        ...
        public void buildIntoRemoteViews(RemoteViews outerView, RemoteViews innerView) {
            ...
            outerView.removeAllViews(C0070R.C0073id.notification_main_column);
            outerView.addView(C0070R.C0073id.notification_main_column, innerView.clone());
            outerView.setViewVisibility(C0070R.C0073id.notification_main_column, 0);
            if (Build.VERSION.SDK_INT >= 21) {
                outerView.setViewPadding(C0070R.C0073id.notification_main_column_container, 0, calculateTopPadding(), 0, 0);
            }
        }

        private void hideNormalContent(RemoteViews outerView) {
            outerView.setViewVisibility(C0070R.C0073id.title, 8);
            outerView.setViewVisibility(C0070R.C0073id.text2, 8);
            outerView.setViewVisibility(C0070R.C0073id.text, 8);
        }

        private int calculateTopPadding() {
            Resources resources = this.mBuilder.mContext.getResources();
            int padding = resources.getDimensionPixelSize(C0070R.dimen.notification_top_pad);
            int largePadding = resources.getDimensionPixelSize(C0070R.dimen.notification_top_pad_large_text);
    ...
    public static class DecoratedCustomViewStyle extends Style {
        ...
        private RemoteViews createRemoteViews(RemoteViews innerView, boolean showActions) {
            ...
            RemoteViews remoteViews = applyStandardTemplate(true, C0070R.layout.notification_template_custom_big, false);
            remoteViews.removeAllViews(C0070R.C0073id.actions);
            boolean actionsVisible = false;
            if (showActions && this.mBuilder.mActions != null && (numActions = Math.min(this.mBuilder.mActions.size(), 3)) > 0) {
                actionsVisible = true;
                for (int i = 0; i < numActions; i++) {
                    remoteViews.addView(C0070R.C0073id.actions, generateActionButton(this.mBuilder.mActions.get(i)));
                }
            }
            if (!actionsVisible) {
                actionVisibility = 8;
            }
            remoteViews.setViewVisibility(C0070R.C0073id.actions, actionVisibility);
            remoteViews.setViewVisibility(C0070R.C0073id.action_divider, actionVisibility);
        ...
        private RemoteViews generateActionButton(Action action) {
            ...
            RemoteViews button = new RemoteViews(this.mBuilder.mContext.getPackageName(), tombstone ? C0070R.layout.notification_action_tombstone : C0070R.layout.notification_action);
            button.setImageViewBitmap(C0070R.C0073id.action_image, createColoredBitmap(action.getIcon(), this.mBuilder.mContext.getResources().getColor(C0070R.C0071color.notification_action_color_filter)));
            button.setTextViewText(C0070R.C0073id.action_text, action.title);
            if (!tombstone) {
                button.setOnClickPendingIntent(C0070R.C0073id.action_container, action.actionIntent);
            }
            if (Build.VERSION.SDK_INT >= 15) {
                button.setContentDescription(C0070R.C0073id.action_container, action.title);
            }
            return button;
        }
    }
