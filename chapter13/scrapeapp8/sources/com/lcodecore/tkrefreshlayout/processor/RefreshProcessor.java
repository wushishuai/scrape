...
public class RefreshProcessor implements IDecorator {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case 2:
                ...
                if (!this.intercepted && Math.abs(dx) <= Math.abs(dy) && Math.abs(dy) > ((float) this.f86cp.getTouchSlop())) {
                ...
                if (this.intercepted) {
                    ...
                    if (!this.f86cp.isPrepareFinishRefresh() && this.f86cp.isStatePTD()) {
                        if (dy < ((float) (-this.f86cp.getTouchSlop())) || !ScrollingUtil.isViewToTop(this.f86cp.getTargetView(), this.f86cp.getTouchSlop())) {
                            this.f86cp.dispatchTouchEventSuper(ev);
                        }
                        ...
                        if (dy > ((float) this.f86cp.getTouchSlop()) || !ScrollingUtil.isViewToBottom(this.f86cp.getTargetView(), this.f86cp.getTouchSlop())) {
                            this.f86cp.dispatchTouchEventSuper(ev);
                        }
                        dy = Math.min(0.0f, Math.max((float) ((-this.f86cp.getMaxBottomHeight()) * 2), dy));
    ...
    private void sendCancelEvent() {
        if (this.mLastMoveEvent != null) {
            ...
            this.f86cp.dispatchTouchEventSuper(MotionEvent.obtain(last.getDownTime(), last.getEventTime() + ((long) ViewConfiguration.getLongPressTimeout()), 3, last.getX(), last.getY(), last.getMetaState()));
        }
    }

    private void sendDownEvent() {
        MotionEvent last = this.mLastMoveEvent;
        this.f86cp.dispatchTouchEventSuper(MotionEvent.obtain(last.getDownTime(), last.getEventTime(), 0, last.getX(), last.getY(), last.getMetaState()));
    }
    ...
    public void onFingerScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY, float velocityX, float velocityY) {
        ...
        if (this.f86cp.isRefreshVisible() && distanceY >= ((float) mTouchSlop) && !this.f86cp.isOpenFloatRefresh()) {
            this.f86cp.getAnimProcessor().animHeadHideByVy((int) velocityY);
        }
        if (this.f86cp.isLoadingVisible() && distanceY <= ((float) (-mTouchSlop))) {
