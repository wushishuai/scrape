...
public class DialogInit {
    ...
    public static int getTheme(@NonNull MaterialDialog.Builder builder) {
        boolean darkTheme = DialogUtils.resolveBoolean(builder.context, C0583R.attr.md_dark_theme, builder.theme == Theme.DARK);
        builder.theme = darkTheme ? Theme.DARK : Theme.LIGHT;
        return darkTheme ? C0583R.style.MD_Dark : C0583R.style.MD_Light;
    }
    ...
    public static int getInflateLayout(MaterialDialog.Builder builder) {
        if (builder.customView != null) {
            return C0583R.layout.md_dialog_custom;
        }
        if (builder.items == null && builder.adapter == null) {
            if (builder.progress > -2) {
                return C0583R.layout.md_dialog_progress;
            }
            if (builder.indeterminateProgress) {
                if (builder.indeterminateIsHorizontalProgress) {
                    return C0583R.layout.md_dialog_progress_indeterminate_horizontal;
                }
                return C0583R.layout.md_dialog_progress_indeterminate;
            } else if (builder.inputCallback != null) {
                if (builder.checkBoxPrompt != null) {
                    return C0583R.layout.md_dialog_input_check;
                }
                return C0583R.layout.md_dialog_input;
            } else if (builder.checkBoxPrompt != null) {
                return C0583R.layout.md_dialog_basic_check;
            } else {
                return C0583R.layout.md_dialog_basic;
            }
        } else if (builder.checkBoxPrompt != null) {
            return C0583R.layout.md_dialog_list_check;
        } else {
            return C0583R.layout.md_dialog_list;
        }
    }

    /* JADX INFO: Multiple debug info for r2v48 int: [D('maxIconSize' int), D('d' android.graphics.drawable.Drawable)] */
    @UiThread
    public static void init(MaterialDialog dialog) {
        ...
        if (builder.backgroundColor == 0) {
            builder.backgroundColor = DialogUtils.resolveColor(builder.context, C0583R.attr.md_background_color, DialogUtils.resolveColor(dialog.getContext(), C0583R.attr.colorBackgroundFloating));
        }
        if (builder.backgroundColor != 0) {
            GradientDrawable drawable = new GradientDrawable();
            drawable.setCornerRadius(builder.context.getResources().getDimension(C0583R.dimen.md_bg_corner_radius));
            drawable.setColor(builder.backgroundColor);
            dialog.getWindow().setBackgroundDrawable(drawable);
        }
        if (!builder.positiveColorSet) {
            builder.positiveColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0583R.attr.md_positive_color, builder.positiveColor);
        }
        if (!builder.neutralColorSet) {
            builder.neutralColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0583R.attr.md_neutral_color, builder.neutralColor);
        }
        if (!builder.negativeColorSet) {
            builder.negativeColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0583R.attr.md_negative_color, builder.negativeColor);
        }
        if (!builder.widgetColorSet) {
            builder.widgetColor = DialogUtils.resolveColor(builder.context, C0583R.attr.md_widget_color, builder.widgetColor);
        }
        if (!builder.titleColorSet) {
            builder.titleColor = DialogUtils.resolveColor(builder.context, C0583R.attr.md_title_color, DialogUtils.resolveColor(dialog.getContext(), 16842806));
        }
        if (!builder.contentColorSet) {
            builder.contentColor = DialogUtils.resolveColor(builder.context, C0583R.attr.md_content_color, DialogUtils.resolveColor(dialog.getContext(), 16842808));
        }
        if (!builder.itemColorSet) {
            builder.itemColor = DialogUtils.resolveColor(builder.context, C0583R.attr.md_item_color, builder.contentColor);
        }
        dialog.title = (TextView) dialog.view.findViewById(C0583R.C0586id.md_title);
        dialog.icon = (ImageView) dialog.view.findViewById(C0583R.C0586id.md_icon);
        dialog.titleFrame = dialog.view.findViewById(C0583R.C0586id.md_titleFrame);
        dialog.content = (TextView) dialog.view.findViewById(C0583R.C0586id.md_content);
        dialog.recyclerView = (RecyclerView) dialog.view.findViewById(C0583R.C0586id.md_contentRecyclerView);
        dialog.checkBoxPrompt = (CheckBox) dialog.view.findViewById(C0583R.C0586id.md_promptCheckbox);
        dialog.positiveButton = (MDButton) dialog.view.findViewById(C0583R.C0586id.md_buttonDefaultPositive);
        dialog.neutralButton = (MDButton) dialog.view.findViewById(C0583R.C0586id.md_buttonDefaultNeutral);
        dialog.negativeButton = (MDButton) dialog.view.findViewById(C0583R.C0586id.md_buttonDefaultNegative);
        ...
        if (builder.icon != null) {
            ...
            Drawable d = DialogUtils.resolveDrawable(builder.context, C0583R.attr.md_icon);
            if (d != null) {
                dialog.icon.setVisibility(0);
        ...
        if (maxIconSize2 == -1) {
            maxIconSize2 = DialogUtils.resolveDimension(builder.context, C0583R.attr.md_icon_max_size);
        }
        if (builder.limitIconToDefaultSize || DialogUtils.resolveBoolean(builder.context, C0583R.attr.md_icon_limit_icon_to_default_size)) {
            maxIconSize2 = builder.context.getResources().getDimensionPixelSize(C0583R.dimen.md_icon_max_size);
        }
        if (maxIconSize2 > -1) {
        ...
        if (!builder.dividerColorSet) {
            builder.dividerColor = DialogUtils.resolveColor(builder.context, C0583R.attr.md_divider_color, DialogUtils.resolveColor(dialog.getContext(), C0583R.attr.md_divider));
        }
        ...
        if (Build.VERSION.SDK_INT >= 14) {
            ...
            if (textAllCaps) {
                textAllCaps = DialogUtils.resolveBoolean(builder.context, C0583R.attr.textAllCaps, true);
            }
        } else {
            textAllCaps = DialogUtils.resolveBoolean(builder.context, C0583R.attr.textAllCaps, true);
        }
        ...
        if (builder.customView != null) {
            ((MDRootLayout) dialog.view.findViewById(C0583R.C0586id.md_root)).noTitleNoPadding();
            FrameLayout frame = (FrameLayout) dialog.view.findViewById(C0583R.C0586id.md_customViewFrame);
            ...
            if (builder.wrapCustomViewInScroll) {
                ...
                int framePadding = r.getDimensionPixelSize(C0583R.dimen.md_dialog_frame_margin);
                ScrollView sv = new ScrollView(dialog.getContext());
                int paddingTop = r.getDimensionPixelSize(C0583R.dimen.md_content_padding_top);
                int paddingBottom = r.getDimensionPixelSize(C0583R.dimen.md_content_padding_bottom);
        ...
        int windowVerticalPadding = builder.context.getResources().getDimensionPixelSize(C0583R.dimen.md_dialog_vertical_margin);
        int windowHorizontalPadding = builder.context.getResources().getDimensionPixelSize(C0583R.dimen.md_dialog_horizontal_margin);
        dialog.view.setMaxHeight(windowHeight - (windowVerticalPadding * 2));
        WindowManager.LayoutParams lp = new WindowManager.LayoutParams();
        lp.copyFrom(dialog.getWindow().getAttributes());
        lp.width = Math.min(builder.context.getResources().getDimensionPixelSize(C0583R.dimen.md_dialog_max_width), windowWidth - (windowHorizontalPadding * 2));
        ...
        if (builder.indeterminateProgress || builder.progress > -2) {
            ...
            if (dialog.progressBar != null) {
                if (Build.VERSION.SDK_INT < 14) {
                    ...
                    IndeterminateCircularProgressDrawable d3 = new IndeterminateCircularProgressDrawable(builder.getContext());
                ...
                if (!builder.indeterminateProgress || builder.indeterminateIsHorizontalProgress) {
                    ...
                    dialog.progressLabel = (TextView) dialog.view.findViewById(C0583R.C0586id.md_label);
                    ...
                    dialog.progressMinMax = (TextView) dialog.view.findViewById(C0583R.C0586id.md_minMax);
    ...
    private static void setupInputDialog(MaterialDialog dialog) {
        ...
        if (dialog.input != null) {
            ...
            dialog.inputMinMax = (TextView) dialog.view.findViewById(C0583R.C0586id.md_minMax);
