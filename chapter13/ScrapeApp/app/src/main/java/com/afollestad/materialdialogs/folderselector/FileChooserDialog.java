...
public class FileChooserDialog extends DialogFragment implements MaterialDialog.ListCallback {
        ...
        for (File fi : contents) {
            if (fi.isDirectory()) {
                ...
                while (true) {
                    ...
                    if (fi.getName().toLowerCase().endsWith(extensions[i].toLowerCase())) {
                        found = true;
                        break;
                    }
                    i++;
    ...
    boolean fileIsMimeType(File file, String mimeType, MimeTypeMap mimeTypeMap) {
        ...
        if (mimeType.substring(mimeTypeDelimiter + 1).equals("*") && (fileTypeDelimiter = fileType.lastIndexOf(47)) != -1 && fileType.substring(0, fileTypeDelimiter).equals(mimeTypeMainType)) {
            return true;
        }
        return false;
    }
