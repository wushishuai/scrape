...
public class FolderChooserDialog extends DialogFragment implements MaterialDialog.ListCallback {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        if (Build.VERSION.SDK_INT >= 23 && ActivityCompat.checkSelfPermission(getActivity(), "android.permission.READ_EXTERNAL_STORAGE") != 0) {
            return new MaterialDialog.Builder(getActivity()).title(C0593R.string.md_error_label).content(C0593R.string.md_storage_perm_error).positiveText(17039370).build();
        }
    ...
    public void createNewFolder() {
        new MaterialDialog.Builder(getActivity()).title(getBuilder().newFolderButton).input(0, 0, false, (MaterialDialog.InputCallback) new MaterialDialog.InputCallback() { // from class: com.afollestad.materialdialogs.folderselector.FolderChooserDialog.4
            ...
            public void onInput(@NonNull MaterialDialog dialog, CharSequence input) {
                ...
                if (!newFi.mkdir()) {
                    Toast.makeText(FolderChooserDialog.this.getActivity(), "Unable to create folder " + newFi.getAbsolutePath() + ", make sure you have the WRITE_EXTERNAL_STORAGE permission or root permissions.", 1).show();
                    return;
                }
    ...
    public static class Builder implements Serializable {
        ...
        String tag;
        @StringRes
        int chooseButton = C0593R.string.md_choose_label;
        @StringRes
        ...
        public Builder allowNewFolder(boolean allow, @StringRes int buttonLabel) {
            ...
            if (buttonLabel == 0) {
                buttonLabel = C0593R.string.new_folder;
            }
