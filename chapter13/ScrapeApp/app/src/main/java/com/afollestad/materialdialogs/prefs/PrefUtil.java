...
class PrefUtil {
    ...
    public static void setLayoutResource(@NonNull Context context, @NonNull Preference preference, @Nullable AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, C0593R.styleable.Preference, 0, 0);
            try {
                useStockLayout = a.getBoolean(C0593R.styleable.Preference_useStockLayout, false);
            } finally {
                a.recycle();
            }
        }
        if (!foundLayout && !useStockLayout) {
            preference.setLayoutResource(C0593R.layout.md_preference_custom);
        }
    }
