...
public class MaterialShapeDrawable extends Drawable implements TintAwareDrawable {
    ...
    @Override // android.graphics.drawable.Drawable
    public void setAlpha(@IntRange(from = 0, m54to = 255) int alpha) {
        this.alpha = alpha;
        invalidateSelf();
    }
    ...
    private void appendEdgePath(int index, Path path) {
        ...
        getEdgeTreatmentForIndex(index).getEdgePath((float) Math.hypot((double) (f - fArr2[0]), (double) (fArr[1] - fArr2[1])), this.interpolation, this.shapePath);
    ...
    private void getPath(int width, int height, Path path) {
        ...
        if (this.scale != 1.0f) {
            ...
            float f = this.scale;
            matrix.setScale(f, f, (float) (width / 2), (float) (height / 2));
