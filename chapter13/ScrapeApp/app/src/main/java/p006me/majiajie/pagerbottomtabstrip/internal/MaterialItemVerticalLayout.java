...
public class MaterialItemVerticalLayout extends ViewGroup implements ItemController {
    ...
    public MaterialItemVerticalLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        this.mSelected = -1;
        this.NAVIGATION_ITEM_SIZE = getResources().getDimensionPixelSize(C1029R.dimen.material_bottom_navigation_height);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int n = getChildCount();
        int heightSpec = View.MeasureSpec.makeMeasureSpec(this.NAVIGATION_ITEM_SIZE, MemoryConstants.f216GB);
        int widthSpec = View.MeasureSpec.makeMeasureSpec(this.NAVIGATION_ITEM_SIZE, MemoryConstants.f216GB);
        for (int i = 0; i < n; i++) {
