...
import com.afollestad.materialdialogs.C0583R;
...
public class MDRootLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C0583R.styleable.MDRootLayout, defStyleAttr, 0);
        this.reducePaddingNoTitleNoButtons = a.getBoolean(C0583R.styleable.MDRootLayout_md_reduce_padding_no_title_no_buttons, true);
        a.recycle();
        this.noTitlePaddingFull = r.getDimensionPixelSize(C0583R.dimen.md_notitle_vertical_padding);
        this.buttonPaddingFull = r.getDimensionPixelSize(C0583R.dimen.md_button_frame_vertical_padding);
        this.buttonHorizontalEdgeMargin = r.getDimensionPixelSize(C0583R.dimen.md_button_padding_frame_side);
        this.buttonBarHeight = r.getDimensionPixelSize(C0583R.dimen.md_button_height);
        this.dividerPaint = new Paint();
        this.dividerWidth = r.getDimensionPixelSize(C0583R.dimen.md_divider_height);
        this.dividerPaint.setColor(DialogUtils.resolveColor(context, C0583R.attr.md_divider_color));
    ...
    public void onFinishInflate() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            View v = getChildAt(i);
            if (v.getId() == C0583R.C0586id.md_titleFrame) {
                this.titleBar = v;
            } else if (v.getId() == C0583R.C0586id.md_buttonDefaultNeutral) {
                this.buttons[0] = (MDButton) v;
            } else if (v.getId() == C0583R.C0586id.md_buttonDefaultNegative) {
                this.buttons[1] = (MDButton) v;
            } else if (v.getId() == C0583R.C0586id.md_buttonDefaultPositive) {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.stackBehavior == StackingBehavior.ALWAYS) {
            ...
            stacked = buttonsWidth > width - (getContext().getResources().getDimensionPixelSize(C0583R.dimen.md_neutral_button_margin) * 2);
        ...
        if (isVisible(this.content)) {
            ...
            if (this.content.getMeasuredHeight() > availableHeight - fullPadding) {
                ...
                availableHeight = 0;
            } else if (!this.reducePaddingNoTitleNoButtons || isVisible(this.titleBar) || buttonBarPadding) {
                this.useFullPadding = true;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (view != null) {
            if (this.drawTopDivider) {
                int y = view.getTop();
                canvas.drawRect(0.0f, (float) (y - this.dividerWidth), (float) getMeasuredWidth(), (float) y, this.dividerPaint);
            }
            if (this.drawBottomDivider) {
                int y2 = this.content.getBottom();
                canvas.drawRect(0.0f, (float) y2, (float) getMeasuredWidth(), (float) (this.dividerWidth + y2), this.dividerPaint);
            }
        }
    }
