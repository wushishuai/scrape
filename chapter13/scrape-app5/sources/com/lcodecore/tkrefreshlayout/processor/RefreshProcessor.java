...
public class RefreshProcessor implements IDecorator {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case 2:
                ...
                if (!this.intercepted && Math.abs(dx) <= Math.abs(dy) && Math.abs(dy) > ((float) this.cp.getTouchSlop())) {
                ...
                if (this.intercepted) {
                    ...
                    if (!this.cp.isPrepareFinishRefresh() && this.cp.isStatePTD()) {
                        if (dy < ((float) (-this.cp.getTouchSlop())) || !ScrollingUtil.isViewToTop(this.cp.getTargetView(), this.cp.getTouchSlop())) {
                            this.cp.dispatchTouchEventSuper(ev);
                        }
                        ...
                        if (dy > ((float) this.cp.getTouchSlop()) || !ScrollingUtil.isViewToBottom(this.cp.getTargetView(), this.cp.getTouchSlop())) {
                            this.cp.dispatchTouchEventSuper(ev);
                        }
                        dy = Math.min(0.0f, Math.max((float) ((-this.cp.getMaxBottomHeight()) * 2), dy));
    ...
    private void sendCancelEvent() {
        if (this.mLastMoveEvent != null) {
            ...
            this.cp.dispatchTouchEventSuper(MotionEvent.obtain(last.getDownTime(), last.getEventTime() + ((long) ViewConfiguration.getLongPressTimeout()), 3, last.getX(), last.getY(), last.getMetaState()));
        }
    }

    private void sendDownEvent() {
        MotionEvent last = this.mLastMoveEvent;
        this.cp.dispatchTouchEventSuper(MotionEvent.obtain(last.getDownTime(), last.getEventTime(), 0, last.getX(), last.getY(), last.getMetaState()));
    }
