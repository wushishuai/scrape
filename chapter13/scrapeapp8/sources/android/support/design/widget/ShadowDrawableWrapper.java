...
public class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    public ShadowDrawableWrapper(Context context, Drawable content, float radius, float shadowSize, float maxShadowSize) {
        super(content);
        this.shadowStartColor = ContextCompat.getColor(context, C0091R.C0092color.design_fab_shadow_start_color);
        this.shadowMiddleColor = ContextCompat.getColor(context, C0091R.C0092color.design_fab_shadow_mid_color);
        this.shadowEndColor = ContextCompat.getColor(context, C0091R.C0092color.design_fab_shadow_end_color);
    ...
    private void drawShadow(Canvas canvas) {
        ...
        if (drawHorizontalEdges) {
            ...
            canvas.drawRect(0.0f, edgeShadowTop, this.contentBounds.width() - (shadowOffset * 2.0f), -this.cornerRadius, paint);
        } else {
            shadowScaleBottom = shadowScaleBottom2;
            shadowScaleTop = shadowScaleTop2;
            shadowScaleHorizontal = shadowScaleHorizontal3;
    ...
    private void buildShadowCorners() {
        ...
        if (shadowRadius > 0.0f) {
            ...
            this.cornerShadowPaint.setShader(new RadialGradient(0.0f, 0.0f, shadowRadius, new int[]{0, this.shadowStartColor, this.shadowMiddleColor, this.shadowEndColor}, new float[]{0.0f, startRatio, startRatio + ((SHADOW_BOTTOM_SCALE - startRatio) / 2.0f), SHADOW_BOTTOM_SCALE}, Shader.TileMode.CLAMP));
        }
