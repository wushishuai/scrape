...
public final class ObservableFlatMap<T, U> extends AbstractObservableWithUpstream<T, U> {
    ...
    static final class MergeObserver<T, U> extends AtomicInteger implements Disposable, Observer<T> {
        ...
        /* JADX WARN: Multi-variable type inference failed */
        /* JADX WARN: Type inference failed for: r2v3, types: [io.reactivex.internal.fuseable.SimplePlainQueue] */
        ...
        /* JADX INFO: Multiple debug info for r0v18 boolean: [D('ex' java.lang.Throwable), D('innerDone' boolean)] */
        void drainLoop() {
            ...
            while (!checkTerminate()) {
                ...
                if (!d || !((svq2 == null || svq2.isEmpty()) && n == 0 && nSources == 0)) {
                    if (n != 0) {
                        ...
                        if (n <= index || inner[index].f153id != startId) {
                            if (n <= index) {
                                index = 0;
                            }
                            int j = index;
                            int i = 0;
                            while (i < n && inner[j].f153id != startId) {
                                j++;
                                if (j == n) {
                                    j = 0;
                                }
        ...
        boolean disposeAll() {
            InnerObserver<?, ?>[] a;
            this.upstream.dispose();
            InnerObserver<?, ?>[] a2 = this.observers.get();
            InnerObserver<?, ?>[] innerObserverArr = CANCELLED;
            if (a2 == innerObserverArr || (a = this.observers.getAndSet(innerObserverArr)) == CANCELLED) {
                return false;
            }
