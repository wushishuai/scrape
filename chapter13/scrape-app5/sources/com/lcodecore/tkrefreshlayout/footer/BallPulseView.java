...
public class BallPulseView extends View implements IBottomView {
    ...
    public BallPulseView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        setLayoutParams(new FrameLayout.LayoutParams(default_size, default_size, 17));
    ...
    protected void onDraw(Canvas canvas) {
        float radius = (((float) Math.min(getWidth(), getHeight())) - (this.circleSpacing * 2.0f)) / 6.0f;
        float x = ((float) (getWidth() / 2)) - ((radius * 2.0f) + this.circleSpacing);
        float y = (float) (getHeight() / 2);
        for (int i = 0; i < 3; i++) {
            canvas.save();
            canvas.translate((radius * 2.0f * ((float) i)) + x + (this.circleSpacing * ((float) i)), y);
