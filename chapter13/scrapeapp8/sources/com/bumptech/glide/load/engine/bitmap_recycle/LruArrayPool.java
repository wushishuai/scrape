...
public final class LruArrayPool implements ArrayPool {
    ...
    public synchronized <T> T getExact(int size, Class<T> arrayClass) {
        return (T) getForKey(this.keyPool.get(size, arrayClass), arrayClass);
    }
    ...
    private <T> ArrayAdapterInterface<T> getAdapterFromType(Class<T> arrayPoolClass) {
        ...
        if (arrayAdapterInterface == null) {
            if (arrayPoolClass.equals(int[].class)) {
                arrayAdapterInterface = (ArrayAdapterInterface<T>) new IntegerArrayAdapter();
            } else if (arrayPoolClass.equals(byte[].class)) {
                arrayAdapterInterface = (ArrayAdapterInterface<T>) new ByteArrayAdapter();
            } else {
    ...
    int getCurrentSize() {
        ...
        for (Class<?> type : this.sortedSizes.keySet()) {
            for (Integer size : this.sortedSizes.get(type).keySet()) {
                currentSize += size.intValue() * ((Integer) this.sortedSizes.get(type).get(size)).intValue() * getAdapterFromType(type).getElementSizeInBytes();
            }
        }
        return currentSize;
    }
