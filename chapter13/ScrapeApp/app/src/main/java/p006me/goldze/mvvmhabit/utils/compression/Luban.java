...
public class Luban {
        ...
        if (i == 1) {
            Observable.just(this.mFile).map(new Function<String, File>() { // from class: me.goldze.mvvmhabit.utils.compression.Luban.4
                public File apply(String s) throws Exception {
                    return Luban.this.firstCompress(new File(s));
                }
            ...
            Observable.just(this.mFile).map(new Function<String, File>() { // from class: me.goldze.mvvmhabit.utils.compression.Luban.8
                public File apply(String s) throws Exception {
                    return Luban.this.thirdCompress(new File(s));
                }
    ...
    public File thirdCompress(@NonNull File file) {
        ...
        if (scale <= 1.0d && scale > 0.5625d) {
            ...
            if (height2 < 1664) {
                ...
                double d3 = (double) (width2 * height2);
                double pow = Math.pow(1664.0d, 2.0d);
                Double.isNaN(d3);
                double size4 = (d3 / pow) * 150.0d;
                if (size4 >= 60.0d) {
                    size3 = size4;
                }
                thumbH = thumbH2;
                thumbW = thumbW2;
                size = size3;
            } else if (height2 >= 1664 && height2 < 4990) {
                int thumbW3 = width2 / 2;
                int thumbH3 = height2 / 2;
                double d4 = (double) (thumbW3 * thumbH3);
                double pow2 = Math.pow(2495.0d, 2.0d);
                Double.isNaN(d4);
                double size5 = (d4 / pow2) * 300.0d;
                if (size5 >= 60.0d) {
                    size3 = size5;
                }
                thumbW = thumbW3;
                thumbH = thumbH3;
                size = size3;
            } else if (height2 < 4990 || height2 >= 10240) {
                int multiple = height2 / 1280 == 0 ? 1 : height2 / 1280;
                int thumbW4 = width2 / multiple;
                int thumbH4 = height2 / multiple;
                double d5 = (double) (thumbW4 * thumbH4);
                thumbH = thumbH4;
                double pow3 = Math.pow(2560.0d, 2.0d);
                Double.isNaN(d5);
                double size6 = (d5 / pow3) * 300.0d;
                if (size6 >= 100.0d) {
                    size2 = size6;
                }
                thumbW = thumbW4;
                ...
                int thumbW5 = width2 / 4;
                int thumbH5 = height2 / 4;
                double d6 = (double) (thumbW5 * thumbH5);
                double pow4 = Math.pow(2560.0d, 2.0d);
                Double.isNaN(d6);
                double size7 = (d6 / pow4) * 300.0d;
                if (size7 >= 100.0d) {
                    size2 = size7;
                }
                thumbW = thumbW5;
            ...
            double d7 = (double) height2;
            Double.isNaN(d7);
            int multiple2 = (int) Math.ceil(d7 / (1280.0d / scale));
            int thumbW6 = width2 / multiple2;
            int thumbH6 = height2 / multiple2;
            double d8 = (double) (thumbW6 * thumbH6);
            Double.isNaN(d8);
            double size8 = 500.0d * (d8 / ((1280.0d / scale) * 1280.0d));
            if (size8 >= 100.0d) {
                size2 = size8;
            }
            thumbW = thumbW6;
            size = size2;
            thumbH = thumbH6;
        } else if (height2 < 1280 && file.length() / PlaybackStateCompat.ACTION_PLAY_FROM_MEDIA_ID < 200) {
            return file;
        } else {
            int multiple3 = height2 / 1280 == 0 ? 1 : height2 / 1280;
            int thumbW7 = width2 / multiple3;
            int thumbH7 = height2 / multiple3;
            double d9 = (double) (thumbW7 * thumbH7);
            Double.isNaN(d9);
            double size9 = (d9 / 3686400.0d) * 400.0d;
            if (size9 >= 100.0d) {
                size2 = size9;
            }
            thumbW = thumbW7;
    ...
    public File firstCompress(@NonNull File file) {
        ...
        if (imgSize[0] <= imgSize[1]) {
            double d = (double) imgSize[0];
            double d2 = (double) imgSize[1];
            Double.isNaN(d);
            Double.isNaN(d2);
            double scale = d / d2;
            if (scale <= 1.0d && scale > 0.5625d) {
                width2 = imgSize[0] > 1280 ? 1280 : imgSize[0];
                height2 = (imgSize[1] * width2) / imgSize[0];
                size2 = (long) 60;
            } else if (scale <= 0.5625d) {
                height2 = imgSize[1] > 720 ? 720 : imgSize[1];
            ...
            double d3 = (double) imgSize[1];
            double d4 = (double) imgSize[0];
            Double.isNaN(d3);
            Double.isNaN(d4);
            double scale2 = d3 / d4;
            if (scale2 <= 1.0d && scale2 > 0.5625d) {
                int height3 = imgSize[1] > 1280 ? 1280 : imgSize[1];
                size = (long) 60;
                height = height3;
                width = (imgSize[0] * height3) / imgSize[1];
            } else if (scale2 <= 0.5625d) {
                int width3 = imgSize[0] > 720 ? 720 : imgSize[0];
                size = maxSize;
                width = width3;
                height = (imgSize[1] * width3) / imgSize[0];
            } else {
                size = 0;
                width = 0;
                height = 0;
    ...
    private Bitmap compress(String imagePath, int width, int height) {
        ...
        int heightRatio = (int) Math.ceil((double) (((float) options.outHeight) / ((float) height)));
        int widthRatio = (int) Math.ceil((double) (((float) options.outWidth) / ((float) width)));
