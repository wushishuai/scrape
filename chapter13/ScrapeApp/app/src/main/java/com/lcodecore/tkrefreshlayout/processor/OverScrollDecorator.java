...
public class OverScrollDecorator extends Decorator {
    ...
    private Handler mHandler = new Handler() { // from class: com.lcodecore.tkrefreshlayout.processor.OverScrollDecorator.1
        ...
        public void handleMessage(Message msg) {
            int mTouchSlop = OverScrollDecorator.this.f85cp.getTouchSlop();
            switch (msg.what) {
                case 0:
            ...
            View mChildView = OverScrollDecorator.this.f85cp.getTargetView();
    ...
    public void onFingerFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if (this.f85cp.enableOverScroll()) {
            ...
            if (dy < (-this.f85cp.getTouchSlop()) && this.preventBottomOverScroll) {
                return;
            }
            if (dy <= this.f85cp.getTouchSlop() || !this.preventTopOverScroll) {
