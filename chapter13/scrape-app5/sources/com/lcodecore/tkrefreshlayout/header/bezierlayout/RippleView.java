...
public class RippleView extends View {
    ...
    public void startReveal() {
        setVisibility(0);
        if (this.va == null) {
            int bigRadius = (int) Math.sqrt(Math.pow((double) getHeight(), 2.0d) + Math.pow((double) getWidth(), 2.0d));
            this.va = ValueAnimator.ofInt(0, bigRadius / 2);
            this.va.setDuration((long) bigRadius);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawCircle((float) (getWidth() / 2), (float) (getHeight() / 2), (float) this.r, this.mPaint);
    }
