    ...
    void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime < SHRINK_OFFSET) {
                ...
                float startTrim2 = ring.getStartingStartTrim();
                scaledTime = startTrim2;
                startTrim = (MATERIAL_INTERPOLATOR.getInterpolation(scaledTime2) * 0.79f) + MIN_PROGRESS_ARC + startTrim2;
            } else {
                float scaledTime3 = (interpolatedTime - SHRINK_OFFSET) / SHRINK_OFFSET;
                startTrim = ring.getStartingStartTrim() + 0.79f;
                scaledTime = startTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation(scaledTime3)) * 0.79f) + MIN_PROGRESS_ARC);
            }
