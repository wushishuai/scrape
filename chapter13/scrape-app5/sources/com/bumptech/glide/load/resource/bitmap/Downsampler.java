...
public final class Downsampler {
    ...
    public Resource<Bitmap> decode(InputStream is, int requestedWidth, int requestedHeight, Options options, DecodeCallbacks callbacks) throws IOException {
        ...
        try {
            return BitmapResource.obtain(decodeFromWrappedStreams(is, bitmapFactoryOptions, (DownsampleStrategy) options.get(DownsampleStrategy.OPTION), decodeFormat, options.get(ALLOW_HARDWARE_CONFIG) != null && ((Boolean) options.get(ALLOW_HARDWARE_CONFIG)).booleanValue(), requestedWidth, requestedHeight, ((Boolean) options.get(FIX_BITMAP_SIZE_TO_REQUESTED_DIMENSIONS)).booleanValue(), callbacks), this.bitmapPool);
        } finally {
        ...
        if (options.inSampleSize == 1 || isKitKatOrGreater) {
            ...
            if (downsampler.shouldUsePool(imageType)) {
                if (sourceWidth < 0 || sourceHeight < 0 || !fixBitmapToRequestedDimensions || !isKitKatOrGreater) {
                    ...
                    expectedHeight = Math.round(((float) ((int) Math.ceil((double) (((float) sourceHeight) / ((float) sampleSize))))) * densityMultiplier);
    ...
    /* JADX INFO: Multiple debug info for r15v16 int: [D('dimensions' int[]), D('powerOfTwoHeight' int)] */
