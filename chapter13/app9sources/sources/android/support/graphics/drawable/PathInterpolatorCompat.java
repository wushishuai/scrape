...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f11mX = new float[min];
            this.f12mY = new float[min];
            ...
            if (((double) Math.abs(this.f11mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f12mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f11mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f12mY[i3] - 1.0f)) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
                        float[] fArr2 = this.f11mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb.append(this.f11mX[0]);
            sb.append(",");
            sb.append(this.f12mY[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f11mX[i6]);
            sb.append(",");
            sb.append(this.f12mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f11mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f11mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f11mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f12mY[i];
        }
        float[] fArr2 = this.f12mY;
        float f3 = fArr2[i];
