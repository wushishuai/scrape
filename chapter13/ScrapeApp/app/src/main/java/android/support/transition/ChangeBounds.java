...
public class ChangeBounds extends Transition {
    ...
    /* JADX INFO: Multiple debug info for r6v4 int: [D('startHeight' int), D('startBounds' android.graphics.Rect)] */
    @Override // android.support.transition.Transition
    @Nullable
                ...
                if (parentMatches(startParent, endParent)) {
                    ...
                    if (!this.mResizeClip) {
                        ...
                        if (numChanges != 2) {
                            ...
                            anim = ObjectAnimatorUtils.ofPointF(view, POSITION_PROPERTY, getPathMotion().getPath((float) startLeft, (float) startTop2, (float) endLeft2, (float) endTop));
                            z = true;
                        } else {
                            final ViewBounds viewBounds = new ViewBounds(view);
                            ObjectAnimator topLeftAnimator = ObjectAnimatorUtils.ofPointF(viewBounds, TOP_LEFT_PROPERTY, getPathMotion().getPath((float) startLeft, (float) startTop2, (float) endLeft2, (float) endTop));
                        ...
                        view = view2;
                        ViewUtils.setLeftTopRightBottom(view, startLeft, startTop2, startLeft + Math.max(startWidth, endWidth), startTop2 + Math.max(startHeight, endHeight));
                        if (startLeft == endLeft2 && startTop2 == endTop) {
                            endLeft = endLeft2;
