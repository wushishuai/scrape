...
public class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    private void setSizeParameters(double progressCircleWidth, double progressCircleHeight, double centerRadius, double strokeWidth, float arrowWidth, float arrowHeight) {
        ...
        double d = (double) screenDensity;
        Double.isNaN(d);
        this.mWidth = d * progressCircleWidth;
        double d2 = (double) screenDensity;
        Double.isNaN(d2);
        this.mHeight = d2 * progressCircleHeight;
        ring.setStrokeWidth(((float) strokeWidth) * screenDensity);
        double d3 = (double) screenDensity;
        Double.isNaN(d3);
        ...
        Animation animation = new Animation() { // from class: com.lcodecore.tkrefreshlayout.header.progresslayout.MaterialProgressDrawable.1
            ...
            public void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                if (interpolatedTime <= 0.5f) {
                    float f = MaterialProgressDrawable.MAX_PROGRESS_ARC - minProgressArc;
                    Interpolator interpolator = MaterialProgressDrawable.MATERIAL_INTERPOLATOR;
                    ring.setStartTrim((f * interpolator.getInterpolation(interpolatedTime / 0.5f)) + startingTrim);
                }
                if (interpolatedTime > 0.5f) {
                    ...
                    ring.setEndTrim((interpolator2.getInterpolation((interpolatedTime - 0.5f) / 0.5f) * minArc) + startingEndTrim);
                }
                ring.setRotation((0.25f * interpolatedTime) + startingRotation);
                MaterialProgressDrawable.this.setRotation((216.0f * interpolatedTime) + ((MaterialProgressDrawable.this.mRotationCount / 5.0f) * MaterialProgressDrawable.FULL_ROTATION));
            }
        };
    ...
    public static class Ring {
        ...
        private void drawTriangle(Canvas c, float startAngle, float sweepAngle, Rect bounds) {
            if (this.mShowArrow) {
                ...
                path2.lineTo((((float) this.mArrowWidth) * f) / 2.0f, ((float) this.mArrowHeight) * f);
                this.mArrow.offset(((float) (cos + exactCenterX)) - inset, y);
                this.mArrow.close();
