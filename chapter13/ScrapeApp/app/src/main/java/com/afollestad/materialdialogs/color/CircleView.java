...
public class CircleView extends FrameLayout {
    ...
    @ColorInt
    public static int shiftColor(@ColorInt int color, @FloatRange(from = 0.0d, m55to = 2.0d) float by) {
        if (by == 1.0f) {
            return color;
        }
    ...
    private void update(@ColorInt int color) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            setForeground(new RippleDrawable(new ColorStateList(new int[][]{new int[]{16842919}}, new int[]{shiftColorUp(color)}), selector, null));
        } else {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.selected) {
            ...
            canvas.drawCircle((float) (getMeasuredWidth() / 2), (float) (getMeasuredHeight() / 2), (float) (whiteRadius - this.borderWidthSmall), this.innerPaint);
            return;
        }
