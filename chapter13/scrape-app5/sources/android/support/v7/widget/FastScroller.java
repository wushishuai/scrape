...
public class FastScroller extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener {
    ...
    @Override // android.support.v7.widget.RecyclerView.OnItemTouchListener
    public void onTouchEvent(@NonNull RecyclerView recyclerView, @NonNull MotionEvent me2) {
        if (this.mState != 0) {
            if (me2.getAction() == 0) {
                boolean insideVerticalThumb = isPointInsideVerticalThumb(me2.getX(), me2.getY());
                boolean insideHorizontalThumb = isPointInsideHorizontalThumb(me2.getX(), me2.getY());
                if (insideVerticalThumb || insideHorizontalThumb) {
                    if (insideHorizontalThumb) {
                        this.mDragState = 1;
                        this.mHorizontalDragX = (float) ((int) me2.getX());
                    } else if (insideVerticalThumb) {
                        this.mDragState = 2;
                        this.mVerticalDragY = (float) ((int) me2.getY());
                    }
                    setState(2);
                }
            } else if (me2.getAction() == 1 && this.mState == 2) {
                this.mVerticalDragY = 0.0f;
                this.mHorizontalDragX = 0.0f;
                setState(1);
                this.mDragState = 0;
            } else if (me2.getAction() == 2 && this.mState == 2) {
                show();
                if (this.mDragState == 1) {
                    horizontalScrollTo(me2.getX());
                }
                if (this.mDragState == 2) {
                    verticalScrollTo(me2.getY());
                }
            }
        }
    }
