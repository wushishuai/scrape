...
public class MaterialDialogUtils {
    ...
    public static MaterialDialog.Builder showIndeterminateProgressDialog(Context context, String content, boolean horizontal) {
        return new MaterialDialog.Builder(context).title(content).progress(true, 0).progressIndeterminateStyle(horizontal).canceledOnTouchOutside(false).backgroundColorRes(C0933R.C0934color.white).keyListener(new DialogInterface.OnKeyListener() { // from class: me.goldze.mvvmhabit.utils.MaterialDialogUtils.4
    ...
    public static MaterialDialog.Builder showBasicDialog(Context context, String content) {
        return new MaterialDialog.Builder(context).title(content).positiveText("确定").negativeText("取消");
    }

    public static MaterialDialog.Builder showBasicDialogNoTitle(Context context, String content) {
        return new MaterialDialog.Builder(context).content(content).positiveText("确定").negativeText("取消");
    }

    public static MaterialDialog.Builder showBasicDialogNoCancel(Context context, String title, String content) {
        return new MaterialDialog.Builder(context).title(title).content(content).positiveText("确定");
    }

    public static MaterialDialog.Builder showBasicDialog(Context context, String title, String content) {
        return new MaterialDialog.Builder(context).title(title).content(content).positiveText("确定").negativeText("取消");
    }

    public static MaterialDialog.Builder showBasicDialogPositive(Context context, String title, String content) {
        return new MaterialDialog.Builder(context).title(title).content(content).positiveText("复制").negativeText("取消");
    }
    ...
    public static MaterialDialog.Builder showInputDialog(Context context, String title, String content) {
        return new MaterialDialog.Builder(context).title(title).content(content).inputType(8289).positiveText("确定").negativeText("取消").input((CharSequence) "hint", (CharSequence) "prefill", true, (MaterialDialog.InputCallback) new MaterialDialog.InputCallback() { // from class: me.goldze.mvvmhabit.utils.MaterialDialogUtils.10
