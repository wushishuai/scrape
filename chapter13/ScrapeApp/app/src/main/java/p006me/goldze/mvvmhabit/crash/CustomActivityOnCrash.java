...
public final class CustomActivityOnCrash {
                    ...
                    public void uncaughtException(Thread thread, Throwable throwable) {
                        ...
                        if (CustomActivityOnCrash.config.isEnabled()) {
                            ...
                            if (CustomActivityOnCrash.hasCrashedInTheLastSeconds(CustomActivityOnCrash.application)) {
                                ...
                                } else if (CustomActivityOnCrash.config.getBackgroundMode() == 2 && (uncaughtExceptionHandler = oldHandler) != null) {
    ...
    public static String getAllErrorDetailsFromIntent(@NonNull Context context, @NonNull Intent intent) {
        ...
        String errorDetails = "Build version: " + getVersionName(context) + " \n";
    ...
    private static Class<? extends Activity> getRestartActivityClassWithIntentFilter(@NonNull Context context) {
        List<ResolveInfo> resolveInfos = context.getPackageManager().queryIntentActivities(new Intent().setAction(INTENT_ACTION_RESTART_ACTIVITY).setPackage(context.getPackageName()), 64);
    ...
    private static Class<? extends Activity> getErrorActivityClassWithIntentFilter(@NonNull Context context) {
        List<ResolveInfo> resolveInfos = context.getPackageManager().queryIntentActivities(new Intent().setAction(INTENT_ACTION_ERROR_ACTIVITY).setPackage(context.getPackageName()), 64);
        if (resolveInfos == null || resolveInfos.size() <= 0) {
            return null;
        }
        try {
